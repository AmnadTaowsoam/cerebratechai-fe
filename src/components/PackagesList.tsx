    ...
    <div className="space-y-8">
        ...
        return (
          <div key={id} className="rounded-2xl border border-white/10 bg-surface/60 p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="text-2xl font-semibold text-text">{getText(p.title, p.titleTh)}</h3>
                <p className="mt-1 text-text-muted">
                  {p.priceFromTHB
                    ? `${isThai ? 'เริ่มที่' : 'From'} ฿${p.priceFromTHB.toLocaleString(priceLocale)}${getPriceUnit(p.priceUnit)}`
                    : (isThai ? 'ดูระดับราคาเพิ่มเติม' : 'See tiers below')}
                  {p.timeline ? ` • ${getTimeline(p.timeline)}` : ''}
                </p>
                {isAssurance && (
                  <p className="mt-1 text-xs text-amber-300">
                    {isThai ? 'แนะนำก่อนขึ้นระบบจริง (Pilot/Production)' : 'Recommended before go-live (Pilot/Production)'}
                  </p>
                )}
                {isCare && (
                  <p className="mt-1 text-xs text-emerald-300">
                    {isThai ? 'ดูแลหลังปล่อยใช้งานจริง' : 'Post-launch optimization & enhancements'}
                  </p>
                )}
              </div>
              ...
              <Link href={pkgPath(p.id, locale) as any} className="rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 px-4 py-2 text-sm text-white hover:from-blue-700 hover:to-purple-700 transition-all">
                {getText(p.ctaLabel, p.ctaLabelTh) || (isThai ? 'ดูรายละเอียด' : 'View details')}
              </Link>
            </div>

            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {p.inScope && (
                <Section title={isThai ? 'รวมในแพ็กเกจ' : 'In scope'} icon={CheckCircle}>
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    {getList(p.inScope, p.inScopeTh).map((x, i) => <li key={i}>{x}</li>)}
                  </ul>
                </Section>
              )}
              {p.outOfScope && (
                <Section title={isThai ? 'ไม่รวม' : 'Out of scope'} icon={XCircle}>
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    {getList(p.outOfScope, p.outOfScopeTh).map((x, i) => <li key={i}>{x}</li>)}
                  </ul>
                </Section>
              )}
              {p.deliverables && (
                <Section title={isThai ? 'สิ่งที่ส่งมอบ' : 'Deliverables'} icon={Package}>
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    {getList(p.deliverables, p.deliverablesTh).map((x, i) => <li key={i}>{x}</li>)}
                  </ul>
                </Section>
              )}
              {p.addons && p.addons.length > 0 && (
                <Section title={isThai ? 'ส่วนเสริม' : 'Add-ons'} icon={Plus}>
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    {getList(p.addons, p.addonsTh).map((x, i) => <li key={i}>{x}</li>)}
                  </ul>
                </Section>
              )}
            </div>
                  ...
                  {p.tiers.map(t => (
                    <div key={t.name} className="rounded-xl border border-white/10 bg-surface/40 p-4">
                      <div className="flex items-baseline justify-between mb-2">
                        <div className="text-text font-semibold">{getText(t.name, t.nameTh)}</div>
                        <div className="text-text font-bold">฿{t.priceTHB.toLocaleString(priceLocale)}</div>
                      </div>
                      <div className="text-sm text-text-muted">
                        {isThai ? `${t.hours} ชม./เดือน • SLA ${t.sla}` : `${t.hours} hrs / mo • SLA ${t.sla}`}
                      </div>
                      {(isThai ? t.bonusTh : t.bonus) && (
                        <div className="mt-1 text-xs text-primary">{isThai ? t.bonusTh : t.bonus}</div>
                      )}
                    </div>
                  ))}
                </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-surface/60 p-6">
        ...
        <ul className="list-disc space-y-1 pl-5 text-text-muted">
          {globalTerms.map((t, i) => <li key={i}>{isThai ? t.th : t.en}</li>)}
        </ul>
      </div>
    </div>
  );
