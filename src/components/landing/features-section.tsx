...
export function FeaturesSection() {
  ...
  const features = [
    {
      ...
      description: 'Advanced ML models for predictive analytics and intelligent automation',
      benefits: ['Predictive Analytics', 'Automated Decision Making', 'Pattern Recognition'],
    ...
    },
    {
      ...
      benefits: ['Object Detection', 'Image Classification', 'Real-time Processing'],
      className: '',
    ...
    },
    {
      ...
      description: 'Modern data pipelines and semantic layers that serve analytics and AI workloads',
      benefits: ['Streaming & Batch Pipelines', 'Data Quality Automation', 'Unified Metrics Layer'],
      className: '',
    ...
    },
    {
      ...
      benefits: ['Stream Processing', 'Event-driven Architecture', 'Microservices'],
      className: '',
    ...
    },
    {
      ...
      benefits: ['Performance Metrics', 'Business Intelligence', 'Custom Dashboards'],
    ...
    },
    {
      ...
      description: 'Operational playbooks for scaling AI capabilities across regions with confidence',
      benefits: ['Multi-region Infrastructure', 'Localization Frameworks', 'Compliance Guardrails'],
      className: '',
  ...
  const handleFeatureClick = (featureTitle: string) => {
    trackCtaClick(`feature-${featureTitle.toLowerCase().replace(/\s+/g, '-')}`, featureTitle);
  };
      ...
      <div className="relative z-10 container mx-auto px-6">
        <div className="mx-auto max-w-3xl text-center">
          <AnimatedGradientText className="mb-6 text-[0.65rem] uppercase tracking-[0.35em] text-white/70 md:text-xs">
            <span>{locale.startsWith('th') ? 'คู่มือการส่งมอบ AI' : 'AI DELIVERY PLAYBOOK'}</span>
          </AnimatedGradientText>
          <h2 className="text-3xl font-bold text-white md:text-4xl">
            <SparklesText text={sectionTitle} className="block text-balance leading-tight" />
          </h2>
          ...
          <BentoGrid>
            {features.map((feature, index) => (
              <BentoCard
                ...
                cta={locale.startsWith('th') ? 'ดูคู่มือการส่งมอบ →' : 'See delivery playbook →'}
                className={cn('min-h-[26rem]', feature.className)}
                background={
                  <>
                    <Particles quantity={index === 0 ? 40 : 24} staticity={28} ease={80} />
        ...
        <div className="mt-16 flex flex-col items-center gap-8 text-center">
          ...
          <ShimmerButton
            ...
            onClick={() => trackCtaClick('features-explore-all', 'Explore All Features')}
          >
            <span>{locale.startsWith('th') ? 'ดูฟีเจอร์ทั้งหมด' : 'Explore All Features'}</span>
