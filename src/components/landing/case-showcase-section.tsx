        ...
        <div className="flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 className="mt-4 text-3xl font-bold text-text md:text-4xl">
              {isThai
                ? 'เห็นผลลัพธ์จริงพร้อมปกป้องความลับลูกค้า'
                : 'See the outcomes while protecting client privacy'}
            </h2>
          ...
          <Button asChild variant="secondary">
            <Link href={`${basePath}/cases` as any}>
              {isThai ? 'ดูเคสทั้งหมด' : 'Browse all case studies'}
            </Link>
          </Button>
        </div>
              ...
              <CardContent className="flex flex-1 flex-col gap-4 text-sm text-text-muted">
                  ...
                  {item.outcomes.slice(0, 3).map((outcome, idx) => (
                    <li key={idx} className="flex items-start gap-2">
                      ...
                      <span className="text-xs">
                        <strong className="text-text">{outcome.label}:</strong>{' '}
                        {outcome.value}
                      </span>
                    </li>
                  ))}
                ...
                <div className="mt-auto flex flex-wrap gap-2 text-xs">
                  ...
                  >
                    ...
                    {item.dataSensitivity === 'Synthetic' && (
                      <Sparkles className="h-3.5 w-3.5" />
                    )}
                    {item.dataSensitivity === 'Public'
                      ? tagLabels.publicData
                      : item.dataSensitivity === 'Anonymised'
                        ? tagLabels.anonymized
                        : tagLabels.synthetic}
                  </span>
                </div>
