...
import { 
  ChevronRight, Home, Brain, Eye, Cpu, Wifi, 
  Layers, BarChart, Microscope, Factory, Leaf, 
  Heart, Building2, Package, FileText, DollarSign, 
  Mail, Users, Shield, Briefcase, 
  BookOpen, HelpCircle, Settings, FileQuestion, Sparkles,
  Newspaper, Workflow, UserPlus
} from 'lucide-react';
import { useNonce } from '@/components/providers/nonce-provider';

// Icon name type for type safety
export type IconName = 
  | 'home' | 'brain' | 'eye' | 'cpu' | 'wifi' | 'layers' | 'barChart' 
  | 'microscope' | 'factory' | 'leaf' | 'heart' | 'building2' | 'package' 
  | 'fileText' | 'dollarSign' | 'mail' | 'users' | 'shield' | 'briefcase' 
  | 'bookOpen' | 'helpCircle' | 'settings' | 'fileQuestion' | 'sparkles' 
  | 'newspaper' | 'workflow' | 'userPlus';
        ...
        {items.map((item, index) => {
          ...
          return (
            <div key={`${item.label}-${index}`} className="flex items-center gap-2">
              ...
              {item.href ? (
                <Link href={item.href as any} className="flex items-center gap-1 hover:text-primary transition-colors">
                  {IconComponent && <IconComponent className="h-4 w-4" />}
                  <span>{item.label}</span>
                </Link>
              ) : (
                <span className="flex items-center gap-1 text-text font-medium" aria-current="page">
                  {IconComponent && <IconComponent className="h-4 w-4" />}
                  <span>{item.label}</span>
                </span>
              )}
...
export function generateBreadcrumbs(pathname: string, locale: string) {
  ...
  if (section === 'services') {
    breadcrumbs.push({ label: isThai ? 'บริการ' : 'Services', href: `${basePath}/services`, icon: 'brain' });
    if (slug) {
      ...
      if (serviceNames[slug]) {
        breadcrumbs.push({ 
          ...
          icon: slug === 'consulting' ? 'brain' : slug === 'development' ? 'cpu' : 'workflow'
        });
      }
    }
    return breadcrumbs;
  ...
  if (section === 'solutions') {
    breadcrumbs.push({ label: isThai ? 'โซลูชัน' : 'Solutions', href: `${basePath}/solutions`, icon: 'brain' });
  ...
  if (section === 'products') {
    breadcrumbs.push({ label: isThai ? 'สินค้าของเรา' : 'Products', href: `${basePath}/products`, icon: 'package' });
    if (slug) {
  ...
  if (section === 'packages') {
    breadcrumbs.push({ label: isThai ? 'แพ็กเกจ' : 'Packages', href: `${basePath}/packages`, icon: 'package' });
    if (slug && packageNames[slug]) breadcrumbs.push({ label: isThai ? packageNames[slug].th : packageNames[slug].en });
    return breadcrumbs;
  }

  if (section === 'cases') {
    breadcrumbs.push({ label: isThai ? 'กรณีศึกษา' : 'Case Studies', href: `${basePath}/cases`, icon: 'fileText' });
    if (slug) breadcrumbs.push({ label: isThai ? 'รายละเอียดกรณีศึกษา' : 'Case Details' });
    return breadcrumbs;
  }

  if (section === 'pricing') {
    breadcrumbs.push({ label: isThai ? 'ราคา' : 'Pricing', icon: 'dollarSign' });
    return breadcrumbs;
  }
  ...
  if (section === 'about') {
    breadcrumbs.push({ label: isThai ? 'เกี่ยวกับเรา' : 'About Us', icon: 'users' });
    return breadcrumbs;
  }

  if (section === 'legal') {
    breadcrumbs.push({ label: isThai ? 'กฎหมาย' : 'Legal', href: `${basePath}/legal`, icon: 'fileQuestion' });
  ...
  if (section === 'industries') {
    ...
    breadcrumbs.push({ label: isThai ? 'อุตสาหกรรม' : 'Industries', href: `${basePath}/industries`, icon: 'factory' });
  ...
  if (section === 'resources') {
    breadcrumbs.push({ label: isThai ? 'แหล่งความรู้' : 'Resources', href: `${basePath}/resources`, icon: 'bookOpen' });
    if (slug) breadcrumbs.push({ label: isThai ? 'รายละเอียด' : 'Details' });
    return breadcrumbs;
  }

  if (section === 'blog') {
    breadcrumbs.push({ label: isThai ? 'บล็อก' : 'Blog', href: `${basePath}/blog`, icon: 'fileText' });
    if (slug) breadcrumbs.push({ label: isThai ? 'บทความ' : 'Article' });
    return breadcrumbs;
  }

  if (section === 'support') {
    breadcrumbs.push({ label: isThai ? 'ซัพพอร์ต' : 'Support', icon: 'helpCircle' });
    return breadcrumbs;
  }

  if (section === 'how-we-work') {
    breadcrumbs.push({ label: isThai ? 'วิธีการทำงาน' : 'How We Work', icon: 'settings' });
    return breadcrumbs;
  }

  if (section === 'trust') {
    breadcrumbs.push({ label: isThai ? 'ความปลอดภัยและความน่าเชื่อถือ' : 'Trust & Security', icon: 'shield' });
    return breadcrumbs;
  }

  if (section === 'careers') {
    breadcrumbs.push({ label: isThai ? 'ร่วมงานกับเรา' : 'Careers', icon: 'briefcase' });
    return breadcrumbs;
  }

  if (section === 'partners') {
    breadcrumbs.push({ label: isThai ? 'พาร์ทเนอร์' : 'Partners', icon: 'userPlus' });
    return breadcrumbs;
  }

  if (section === 'faq') {
    breadcrumbs.push({ label: isThai ? 'คำถามที่พบบ่อย' : 'FAQ', icon: 'helpCircle' });
    return breadcrumbs;
  }

  if (section === 'news') {
    breadcrumbs.push({ label: isThai ? 'ข่าว' : 'News', href: `${basePath}/news`, icon: 'newspaper' });
    return breadcrumbs;
  }

  // Handle dashboard and other pages
  if (section === 'dashboard') {
    breadcrumbs.push({ label: isThai ? 'แดชบอร์ด' : 'Dashboard', icon: 'barChart' });
    return breadcrumbs;
  }

  if (section === 'register') {
    breadcrumbs.push({ label: isThai ? 'สมัครสมาชิก' : 'Register', icon: 'users' });
    return breadcrumbs;
  }

  if (section === 'login') {
    breadcrumbs.push({ label: isThai ? 'เข้าสู่ระบบ' : 'Login', icon: 'shield' });
    return breadcrumbs;
  }
