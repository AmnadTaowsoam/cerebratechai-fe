...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  const t = await getTranslations({ locale: params.locale, namespace: 'industries.healthcare' });
...
export default async function HealthcarePage({ params }: { params: { locale: string } }) {
  const t = await getTranslations({ locale: params.locale, namespace: 'industries.healthcare' });
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="relative py-24 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-cyan-500 via-teal-500 to-blue-900 opacity-20"></div>
        <div className="container mx-auto px-4 relative">
          <div className="text-center">
            <h1 className="text-5xl md:text-6xl font-bold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-teal-500 bg-clip-text text-transparent">
              {t('title')}
            </h1>
            <p className="text-xl md:text-2xl opacity-90 mb-8 max-w-4xl mx-auto text-teal-100">
              {t('description')}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="#solutions"
                className="px-8 py-4 bg-white text-teal-600 rounded-lg font-semibold hover:bg-opacity-90 transition min-w-[180px]"
              >
                {t('solution1')}
              </Link>
              <Link
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            <div className="bg-slate-100 p-8 rounded-2xl border border-slate-200 hover:border-teal-500 transition">
            ...
            <div className="bg-slate-100 p-8 rounded-2xl border border-slate-200 hover:border-cyan-500 transition">
              <div className="flex items-center justify-center mb-4">
      ...
      <section className="py-20 bg-slate-100">
        <div className="container mx-auto px-4">
          ...
          <Link
            href="#"
            className="inline-flex items-center gap-2 px-8 py-4 bg-teal-600 text-white rounded-lg font-semibold hover:bg-teal-700 transition"
          >
            {t('cta')}
          </Link>
        </div>
      ...
      <section className="py-20 bg-gradient-to-r from-teal-500 to-cyan-500">
        <div className="container mx-auto px-4 text-center text-white">
          ...
          <Link
            href="#"
            className="inline-flex items-center gap-2 px-8 py-4 bg-white text-teal-600 rounded-lg font-semibold hover:bg-teal-700 transition"
          >
            {t('cta')}
          </Link>
        </div>
