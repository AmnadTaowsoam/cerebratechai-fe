...
export default function ResourcesPage() {
  ...
  const categories = useMemo(
    () => [
      ...
      {
        slug: 'whitepaper' as const,
        label: isThai ? 'ไวท์เปเปอร์' : 'Whitepapers',
        count: counts.whitepaper,
      },
      {
        slug: 'webinar' as const,
        label: isThai ? 'เว็บบินาร์' : 'Webinars',
        count: counts.webinar,
      },
      {
        slug: 'ebook' as const,
        label: isThai ? 'อีบุ๊ก' : 'E-books',
        count: counts.ebook,
      },
      {
        slug: 'template' as const,
        label: isThai ? 'เทมเพลต' : 'Templates',
  ...
  const openDownloadGate = (resource: ResourceItem) => {
    ...
    setGateOpen(true);
    trackCtaClick(
      `resource-download-${resource.slug}`,
  ...
  return (
    <div className="bg-bg">
      ...
      <MagicHero
        ...
        title={
          isThai
            ? 'เครื่องมือและบทความเพื่อทำ AI ให้ใช้งานจริง'
            : 'Practical resources for production-ready AI'
        }
        description={
          isThai
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 text-xs text-primary mb-2">
                          <span className="px-2 py-1 rounded-full bg-primary/10 text-primary">
                            {isThai
                              ? resource.category.th
                              : resource.category.en}
                          </span>
                    ...
                    <div className="flex gap-2">
                      {resource.downloadUrl ? (
                        <Button
                          size="sm"
                          className="flex-1"
                          onClick={() => openDownloadGate(resource)}
                        >
                        ...
                        <Button size="sm" className="flex-1" asChild>
                          <Link
                            href={
                              `${basePath}/resources/${resource.slug}` as any
                            }
                            onClick={() =>
                              trackCtaClick(
                                `resource-watch-${resource.slug}`,
                                `Open ${resource.slug}`
                              )
                            }
                          >
          ...
          <Card className="border border-hairline bg-gradient-to-br from-primary/10 via-surface to-accent/10">
            <CardContent className="p-8 text-center">
              <h2 className="text-2xl font-bold text-text mb-4">
                {isThai ? 'รับทรัพยากรใหม่ก่อนใคร' : 'Get New Resources First'}
              </h2>
              ...
              {showSuccess ? (
                <p className="mt-4 text-sm text-success">
                  {isThai
                    ? 'สำเร็จ! ขอบคุณที่สมัครรับข่าวสาร'
      ...
      <section className="py-20">
        <div className="container mx-auto px-6 text-center">
          <h2 className="text-2xl font-bold text-text mb-4">
            {isThai ? 'อยากให้ช่วยเลือกทรัพยากรให้?' : 'Need help choosing?'}
          </h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button asChild variant="outline" size="lg">
              <Link href={`${basePath}/blog` as any}>
                {isThai ? 'อ่านบทความ' : 'Read the blog'}
              </Link>
            </Button>
          </div>
        </div>
