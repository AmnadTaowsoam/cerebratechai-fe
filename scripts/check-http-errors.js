...
const CLIENT_ERROR_PATTERNS = [
  ...
  {
    pattern:
      /(?:method:\s*['"`](POST|PUT)['"`]|fetch\([^,]+,\s*\{[^}]*method:\s*['"`](POST|PUT)['"`])/gi,
...
const SERVER_ERROR_PATTERNS = [
  ...
  {
    pattern:
      /export\s+async\s+function\s+(GET|POST|PUT|DELETE|PATCH)\([^)]*\)\s*\{[^}]*return\s+Response/g,
...
function checkFile(filePath) {
  try {
    ...
    const apiMatches =
      content.matchAll(API_ROUTE_PATTERNS[0]) ||
      content.matchAll(API_ROUTE_PATTERNS[1]);
    ...
    const errorBoundaryMatches = content.matchAll(
      SERVER_ERROR_PATTERNS[1].pattern
    );
...
function checkErrorBoundaries() {
  ...
  if (errorBoundaries.length > 0) {
    console.log(
      `✅ Found ${errorBoundaries.length} ErrorBoundary components:\n`
    );
    ...
    console.log(
      '⚠️  No ErrorBoundary components found. Consider adding error boundaries to handle runtime errors.\n'
    );
  }
}
